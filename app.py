...
# ... (Include your existing parse_chat, analyze_chat, and format_results functions here) ...
...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            temp_path = "temp_chat_file.txt"
            file.save(temp_path)
            
            # Process the file
            messages = parse_chat(temp_path)
...
def download_results():
    ...
    buffer = io.BytesIO()
    buffer.write(results.encode('utf-8'))
    buffer.seek(0)
    return send_file(buffer, as_attachment=True, download_name='chat_analysis_results.txt', mimetype='text/plain')
